...
- **Toast.receipt**: `secondaryLabel` / `onSecondary` add the quieter second action (`View`) beside `Undo`. The two travel together — asserted in the factory and checked again at build, because an assert is gone in release and a control a screen reader cannot announce is worse than none. Either action answers the toast: whichever is tapped first reports, cancels the auto-dismiss and dismisses, and the other is locked out. `View` is an `InkWell` with its ink turned off, so the whole 48 dp box answers a tap and keyboard, D-pad and switch users can reach it; it renders in `bodyLargeSemiBold` at 85 % of `textLink` (CA-1042)
- **CoreDisplayArea**: `onPreviousSessionTapped(String id)` reports a tap on a previous-session card so the consumer can restore that tape, with `restoreSemanticsLabel` naming the control for a screen reader — asserted non-null whenever the callback is given. `CoreHistorySessionData.id` is the new optional identifier; a session without one, or with an empty one, stays the plain card older callers already pass. Ids must be unique across `previousSessions`. The card is an `InkWell` with its ink suppressed but its focus highlight kept, so keyboard, D-pad and switch users can reach it, and its contents sit under an `IgnorePointer`, an `ExcludeFocus` and a `MergeSemantics`, so a chip in a past session is inert to touch, to the keyboard and to a screen reader alike — it can neither win the gesture arena, nor stay a tab stop, nor sit in the tree as an enabled button that answers nothing (CA-1042)
...
- Showcases: the display-area demo answers a restore tap with `CoreToast.showReceipt` naming the session that came back, so the tappable card and the receipt — the two halves of CA-1042 — demo together instead of echoing through a Material `SnackBar` (CA-1042)

### ⚠️ Behaviour change

- **Toast**: an untitled toast is announced once instead of twice. Its semantics label is the title alone, and the description reaches the screen reader through the text it labels; before, an untitled toast used its description as the node label as well, so the description was read out and then read again (CA-1042)
- **Toast.receipt**: the second action renders at 85 % of `textLink` rather than the design's 75 %. At 75 % it composites to `#3C839D` on the receipt surface — 4.02:1, under the 4.5:1 WCAG AA floor — and the component's own contrast guideline fails on it. Open with design
...
- **CoreKeyboard**: `onGroupsReordered` and `reorderSemanticsLabelBuilder` pass through to the built-in "View all" sheet. The keyboard keeps a rebuild hook rather than an order, so when the consumer applies a reorder — as a new `allGroups` list or in place — the sheet that is still open re-renders in the new order. Both indices reported by the callback are in range for `allGroups`
...
- **CoreDisplayArea**: `dependentKeys: List<CoreDependentKeyData>` renders a row of pills under the value instead of the single `dependentKeyLabel` / `dependentKeyValue` pair, because an answer can depend on more than one assumption at once — a cost shows `Rate` and `Waste` together (CA-1037)
  ...
  - Each pill is a `CoreButton` exposing `button: true` with its label (or the consumer's `semanticsLabel`), drawn with a 1 px `lineMid` hairline rather than the secondary button's 2 px outline, as in the Figma Dependent Key Chip spec (`66225:151236`); the row holds no default strings, in line with the 0.15.0 localization rule
...
- **CoreButton**: optional `borderColor` and `borderWidth` override the variant's border in every state (`null` keeps the variant's own), plus `CoreButton.hairlineBorderWidth` (1 px) for chip-style edges such as the dependent-key pill
...
- **SuggestionData**: gains `kind` — `SuggestionKind { deterministic, predictive, bind, conversion, memory }`, default `predictive` — so the app can attach the right accept behaviour without re-deriving it from label text, and an optional `semanticsLabel` that overrides the chip's announced text (`'12.57yd²'` → "Convert to 12.57 square yards"). A suggestion update that only swaps a `kind` or `semanticsLabel` now also collapses an expanded area (CA-1036)
- **CoreSuggestionArea**: a `SuggestionKind.bind` offer renders with the dashed offer outline and a trailing `bindSuffix` (default `'?'`, overridable per locale) appended to its last text segment — `Height: 8ft ?` — because accepting a bind relabels an existing chip instead of adding a result, and the look says so. The looks follow the Figma Suggestion Strip Chip spec (`66225:151222`): a `deterministic` rung renders with the 2 px `lineHighlight` outline, a `memory` recall with a leading `CoreIcons.history` icon (an empty `label` shows the value alone), and the bind chip keeps the strip's plain fill
- **CoreChip**: gains `outline` (`CoreChipOutline { solid, dashed, highlight }`, default `solid`) and `semanticsLabel`. The dashed outline reuses `CoreDashedBorderDecoration` (0.16.0) as the chip's `foregroundDecoration` over the chip's usual fill, with the solid border painted transparent so the chip measures the same and the stroke following the focus border width; `CoreChipTheme.dashedOutline(outline:, isFocused:, colors:)` resolves it. The highlight outline is a solid `lineHighlight` border at the focus width (`CoreChipTheme.borderWidthFor(outline:)`), with the padding giving the extra width back so the chip measures the same
...
- CoreChip widget tests: default solid outline, dashed and highlight chip text, `semanticsLabel` override, and theme resolution for the dashed chip's unchanged fill and transparent border, the highlight border colour, width and state priority, and the focus stroke width in light and dark
- Suggestion area widget tests: deterministic renders the highlight outline; memory renders the history icon and hides an empty label
- Goldens: `suggestion_area_bind_{light,dark}.png` (bind offer beside a deterministic chip) and `core_chip_outline_{light,dark}.png` (highlight, solid and dashed at large, dashed at medium), viewport scoped to one chip row; existing suggestion area and chip goldens are byte-identical
...
- **CoreCalculatorChip**: three new `CoreCalculatorChipType` variants for the calculator tape, so the app no longer has to borrow `disabled` for an answer or hand-build a chip for an error (CA-1034)
  ...
  - `error` — `alertRed` fill inside a new `alertRedOutline` stroke (`red200` light / `red700` dark, the Figma spec's error border) with a regular-weight `textDark` value for the dimension-error chip that backspace repairs
  - `editable`, `dashed` and `error` carry `CoreShadows.small` as in the Figma Calculator Chip spec (`58781:24269`); the filled `disabled`, `active` and `result` chips stay flat
...
- **AppColorsExtension**: new `alertRedOutline` token (`red200` light / `red700` dark) for the stroke around an `alertRed` surface
...
- **KeyType / GroupNameType**: both gain a required `id` — a stable, locale-independent identifier separate from the displayed `label`. `GroupNameType` equality and `hashCode` now key on `id` alone, so a group stays the same group (and the same `Map` key) when the app switches locale and re-renders its label. `KeyType.groupName` is documented as holding the group's `id` (CA-898)
  ...
  - `GroupNameType.id` must be non-empty and unique among sibling groups — equality depends on nothing else, so a shared `id` collapses two groups into one `Map` key. The constructor asserts against the empty string, the shape a missed migration takes
...
- `GroupNameType` identity tests: equal across differing labels, unequal across differing ids, still resolving as a `Map` key when the label is localized, and an empty `id` rejected at construction
- `KeyType` identity test: `id` is independent of the displayed `label`
